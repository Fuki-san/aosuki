...
class TalkController extends Controller
{
    ...
    public function update(UpdateTalkRequest $request, string $id)
    {
        ...
        if ($request->user()->cannot('update', $R)) {
            return redirect()->route('Rs.show', $R)
                ->withErrors('自分の記事以外は更新できません');
        }

        $file = $request->file('image');
        if ($file) {
            $delete_file_path = 'images/Rs/' . $R->image;
            $R->image = date('YmdHis') . '_' . $file->getClientOriginalName();
        }
        $R->fill($request->all());
        ...
        try {
            ...
            if ($file) {
                // 画像アップロード
                if (!Storage::putFileAs('images/Rs', $file, $R->image)) {
                    // 例外を投げてロールバックさせる
                    throw new \Exception('画像ファイルの保存に失敗しました。');
                }
                ...
                if (!Storage::delete($delete_file_path)) {
                    //アップロードした画像を削除する
                    Storage::delete('images/Rs/' . $R->image);
                    //例外を投げてロールバックさせる
