        ...
        <div class="my-4 p-4 rounded-lg bg-blue-200">
            <ul>
                @foreach ($chats as $chat)
                    <p class="text-xs @if($chat->user_name == 'Fuki') text-right @endif">{{ $chat->created_at }} @ {{ $chat->user_name }}</p>
                    <li class="w-max mb-3 p-2 rounded-lg bg-blue-200 relative @if($chat->user_name == 'Fuki') self ml-auto @else other @endif">{{ $chat->message }}</li>
                @endforeach
            </ul>
        </div>
