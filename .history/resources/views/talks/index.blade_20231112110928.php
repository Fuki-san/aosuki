            ...
            @foreach ($talks as $talk)
                <article class="w-full px-4 md:w-1/2 text-xl text-gray-800 leading-normal">
                    <a href="{{ route('talks.show', $talk) }}">
                        <h2 class="font-bold font-sans break-normal text-gray-900 pt-6 pb-1 text-3xl md:text-4xl break-words">{{ $talk->title }}</h2>
                        ...
                        <p class="text-gray-700 text-base">{{ Str::limit($talk->body, 50) }}</p>
                    </a>
                </article>
            @endforeach
        </div>
