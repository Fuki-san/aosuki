    ...
    <div class="container lg:w-3/4 md:w-4/5 w-11/12 mx-auto my-8 px-8 py-4 bg-white shadow-md">
        ...
        <article class="mb-2">
            <h2 class="font-bold font-sans break-normal text-gray-900 pt-6 pb-1 text-3xl md:text-4xl break-words">
                {{ $talk->nickname }}</h2>
            {{-- <h3>{{ $talk->user->nickname }}</h3> --}}
            <p class="text-sm mb-2 md:text-base font-normal text-gray-600">
                <span
                    class="text-red-400 font-bold">{{ date('Y-m-d H:i:s', strtotime('-1 day')) < $talk->created_at ? 'NEW' : '' }}</span>
                {{ $talk->created_at }}
            </p>
        ...
        <div class="flex flex-row text-center my-4">
            ...
            @endcan
            @can('delete', $talk)
                <form action="{{ route('talks.destroy', $post) }}" method="post">
                    @csrf
                    @method('DELETE')
